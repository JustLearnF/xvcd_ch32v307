...
int handle_data(SOCKET fd, unsigned long frequency) {
  ...
  do {
    char cmd[16];
    unsigned char buffer[VECTOR_IN_SZ], result[VECTOR_IN_SZ];
    ...
    if (memcmp(cmd, "ge", 2) == 0) {  // 获取信息
      ...
      // Convert the 4-byte little endian integer after "settck:" to be an
      // integer
      ...
      actPeriod = io_set_period(iIndex_CH32, (unsigned int)period);

      if (actPeriod < 0) {
        fprintf(stderr, "Error while setting the JTAG TCK period\n");
        actPeriod = period; /* on error, simply echo back the period value so
                               client while not try to change it*/
      }
    ...
    if (vlevel > 2) {
      ...
      for (i = 0; i < nr_bytes; i++) printf("%02x ", buffer[i]);
      printf("\n");
    }
    ...
    // "capture_ir"/"capture_dr" cycle after reading IR/DR which unfortunately
    // sets IR to the read-out IR value. Just ignore these transactions.
    if ((jtag_state == exit1_ir && len == 5 && buffer[0] == 0x17) ||
      ...
      if (verbose) {
        for (i = 0; i < nr_bytes; ++i) printf("%02x ", result[i]);
      }
    }
...
void usage(void) {
  const char use[] =
      ...
      " -a, --address <host add xxx>   specify host address, default is "
      "127.0.0.1\n"
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
      usage();
               ...
               !strcmp(argv[i], "--address") && i + 1 < argc) {
      strcpy(szAddress, argv[i + 1]);
      ++i;
    } else  // port
      if (!strcmp(argv[i], "-p") ||
          !strcmp(argv[i], "--port") && i + 1 < argc) {
        iPort = atoi(argv[i + 1]);
        ++i;
      } else if (!strcmp(argv[i], "-i") ||
                 !strcmp(argv[i], "--index") && i + 1 < argc) {
        iIndex_CH32 = atoi(argv[i + 1]);
        ++i;
      } else if (!strcmp(argv[i], "-s") ||
                 !strcmp(argv[i], "--speed") && i + 1 < argc) {
  ...
  address.sin_addr.s_addr =
      strcmp(szAddress, "") ? inet_addr(szAddress) : INADDR_ANY;
