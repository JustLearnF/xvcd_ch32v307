...
int io_init(unsigned int index) { return usb20jtag_init(); }
...
int io_set_period(unsigned int index, unsigned int period) {
  ...
  int RetVal;
  long clock_rate =
      (is_use_default_speed) ? DEFAULT_JTAG_SPEED : 1000000000 / period;
  int speed_clock[] = {MHZ(2.25), MHZ(4.5), MHZ(9),
                       MHZ(18),    MHZ(36),   MHZ(72)};
  // int speed_clock[] = {MHZ(1.875), MHZ(3.75), MHZ(7.5),
  //                      MHZ(15),    MHZ(30),   MHZ(60)};
  // int speed_clock[] = {MHZ(1.875), MHZ(3.75), MHZ(7.5), MHZ(15), MHZ(30),
  // MHZ(60)};
  ...
  for (i = 0; i < sizeof(speed_clock) / sizeof(int); i++) {
    if ((clock_rate >= speed_clock[i]) && (clock_rate <= speed_clock[i + 1])) {
      if (i < 5)
        ...
        clockIndex = i;
      RetVal = USB20Jtag_ClockRateInit(iIndex, clockIndex);
      if (!RetVal) {
        return -1;
      }
      printf("CH32v307 Set Clock : %d.\n", speed_clock[clockIndex]);
      break;
    }
  }

  period = MHZ(1000) / speed_clock[i];
  // if (period > 10) period = period - (period % 10);
  return period;
}
...
int writeTDI(const uint8_t* tx, uint8_t* rx, uint32_t len, bool end) {
  ...
  uint8_t cmd = (rx != NULL) ? USB20Jtag_CMD_JTAG_DATA_SHIFT_RD
                             : USB20Jtag_CMD_JTAG_DATA_SHIFT;
  while (tptr < txend) {
    ...
    int ret = usb_xfer(chunk + 3, (rx) ? chunk + 3 : 0, &actual_length,
                       rx == 0 && get_obuf_length());
    if (!rx) continue;
    if (ibuf[0] != USB20Jtag_CMD_JTAG_DATA_SHIFT_RD) {
      printf("ibuf[0] != USB20Jtag_CMD_JTAG_DATA_SHIFT_RD\n");
    }
    unsigned size = ibuf[1] + ibuf[2] * 0x100;
    if (ibuf[0] != USB20Jtag_CMD_JTAG_DATA_SHIFT_RD ||
        actual_length - 3 != size) {
  ...
  if (bits == 0) return EXIT_SUCCESS;
  cmd = (rx) ? USB20Jtag_CMD_JTAG_BIT_OP_RD : USB20Jtag_CMD_JTAG_BIT_OP;
  ...
  *ptr++ = x & ~TCK_H;
  ...
  if (!rx) return EXIT_SUCCESS;

  unsigned size = ibuf[1] + ibuf[2] * 0x100;
  if (ibuf[0] != USB20Jtag_CMD_JTAG_BIT_OP_RD || actual_length - 3 != size) {
    printf("writeTDI: invalid read data\n");
  }
...
int io_scan(const unsigned char* TMS, const unsigned char* TDI,
            unsigned char* TDO, int bits, int state) {
  unsigned char v;
  ...
  if ((state == shift_dr || state == shift_ir) &&
      is_all_zero(TMS, (bits + 7) / 8)) {
    // set tck to low
    v = TCK_L | TMS_L | TDI_L;
    unsigned char settck[] = {0xD2, 0x01, 0x00, 0x00};
    int len = 4;
    if (!USB20Jtag_Write(settck, &len) || len != 4) return -1;
    len = 3;
    // D2 01 00
    if (!USB20Jtag_Read(settck, &len) || len != 3) return -1;
    if (is_direct_spi)
      return writeTDI(TDI, NULL, bits, false);
    else
  ...
  while (DI < bits) {
    ...
    CmdBuffer[BI++] = USB20Jtag_CMD_JTAG_BIT_OP_RD;
    ...
    // 添加用于处理大包数据时组包操作参数
    RetVal = USB20Jtag_Write(CmdBuffer, &BI);
    if (!RetVal) {
      printf("CH32v307 Write data failed.\n");
      return -1;
    }

    Txlen = length + 3;
    RetVal = USB20Jtag_Read(rbuffer, &Txlen);
    if (!RetVal) {
      printf("CH32v307 read data failed.\n");
      return -1;
    }
    memcpy(&buffer[DI], &rbuffer[USB20Jtag_CMD_HEADER], Txlen);

    if (Txlen != (length + 3)) {
      RetVal = USB20Jtag_Read(rbuffer, &BI);
      if (!RetVal) {
        printf("CH32v307 read data failed.\n");
        return -1;
      }
...
void io_close(void) { usb20jtag_quit(); }
...
int usb_xfer(unsigned wlen, unsigned rlen, unsigned* ract, bool defer) {
  ...
  if (wlen) {
    actual_length = wlen;
    if (!USB20Jtag_Write(obuf, &actual_length) || actual_length != wlen) {
      printf("write fail.\n");
      return -1;
    }
  }
  ...
  if (rlen) {
    while (rlen) {
      actual_length = rlen;
      if (!USB20Jtag_Read(pibuf, &actual_length)) {
        printf("read fail.\n");
        return -1;
      }
      rlen -= actual_length;
